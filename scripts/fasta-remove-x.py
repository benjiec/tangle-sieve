#!/usr/bin/env python3

import argparse
import sys

from Bio import SeqIO

from tangle import open_file_to_read, open_file_to_write


def remove_x_sequences(input_fasta, output_fasta):
    with (
        open_file_to_read(input_fasta) as input_stream,
        open_file_to_write(output_fasta, "wt") as output_stream,
    ):
        for record in SeqIO.parse(input_stream, "fasta"):
            sequence = str(record.seq)
            if "X" in sequence:
                continue
            output_stream.write(f">{record.description}\n{sequence}\n")


def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Remove sequences containing X from a FASTA file."
    )
    parser.add_argument("input_fasta")
    parser.add_argument("output_fasta")
    args = parser.parse_args(argv)

    remove_x_sequences(args.input_fasta, args.output_fasta)
    return 0


if __name__ == "__main__":
    sys.exit(main())
